...
> - **가장 먼저**: [`sessions/바이브코딩_커리큘럼_기준안.md`](sessions/바이브코딩_커리큘럼_기준안.md)의 **§4(대상)·§5(과정 범위)·§6(공통 교육 원칙)**. 이 세 절은 덱 설계에 직접 구속력이 있다 — 대상 정의, 다룰 수 있는 언어·도구, "한 화면에는 한 단계만"·"라이브 코딩 금지" 같은 원칙이 여기서 나온다. **커리큘럼 정본과 이 스킬의 규칙이 어긋나면 언제나 기준안이 이긴다.**
