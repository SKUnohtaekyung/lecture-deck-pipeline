...
```
- **coded**: no
- **density_note**: 전폭 밴드라 본문 22px에서 줄당 폭이 넓어 3줄이면 밴드 세로 250px에 안착. 두 밴드 높이를 flex로 균등 분할해 위계 0. 색-문법: 후 밴드에만 `.callout.green`(개선/안전)을 절제해 쓰되 전 밴드는 중립 `--surface`로 두어 "값 판단"을 색이 대신 말하게.
