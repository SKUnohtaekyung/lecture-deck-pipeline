...
## 상태
- [x] 8 패밀리 MD 항목 50개 · 역인덱스 `by-shape.md` · 균형 감사(50/50, split 8%)
- [x] 코드 코어 `patterns.css` + `catalog.html` **8항목**(L-fb-statement·L-ct-definition·L-td-table·L-td-metric-row·L-gm-quad·L-cs-central-contrast·L-vf-numbered-stack·L-dc-hero) — 브라우저 검증: 오버플로0·콘솔0·토큰·aria
- [ ] 나머지 42개 `<section>` catalog.html로 확장(버전 올리며)
