...
> ⛔ **2026-08-04 정정 — 위치와 git 상태가 둘 다 바뀌었다.** 이 문서 본문의 시점 기록은 고치지 않았으니 아래를 기준으로 읽는다.
> - **위치**: `reports/create-slides-quality/` → **`_dev/설계기록/품질감사-2026-07/`**로 이동했다(폴더 통째, 76파일 무손실). 본문과 다른 문서에 남은 구경로 인용은 같은 폴더 `README.md`의 구경로→신경로 대응표에서 현재 위치를 찾는다.
> - **git**: "커밋하지 않는다"·"`reports/` 전체 **미추적**"은 2026-07-27 시점의 사실이다. 이후 커밋됐고 **2026-08-04 현재 76개 파일 전부 git 추적 중**이다.
> - **파일 수**: 본문의 26개는 이 문서를 쓰기 직전 시점 값이다. 이후 `samples_v5A/`·`samples_v5B/` 등이 추가돼 현재는 **76개 · 1,199,510 bytes**다.
