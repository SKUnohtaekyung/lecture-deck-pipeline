...
## 0.8.0-wip — 아젠다 1열 기본 전환·상시 규칙 3종 승격 (2026-07-21)
- **아젠다 1열 연속이 기본(v3.1) — 0.6.0의 "4개 초과 2열" 규칙 폐지**: 아젠다는 "오늘 배울 순서"를 전달하는 **시퀀스**인데, `an-2col`은 `::after` 세로 연결선을 끄기 때문에 2열이 되는 순간 1→N의 흐름이 끊긴다(사용자 지적: "1,2,3,4,5,6이 연속적으로 한 번에 쭉 내려가야 한다"). **N≤6까지 1열로 수용**한다 — 실측 근거: 항목 높이 = max(배지 64, `.an-h` 29 + 마진 5 + `.an-d` 28 + padding-top 4 = 66) = 66px → 6개 = 66×6 + gap16×5 = **476px < 밴드 514px**. 컬럼 가용폭 568px이라 `.an-h`는 한글 24자까지 1줄이므로 **수동 `<br>` 금지**(줄바꿈은 폭이 정한다). `an-2col` CSS는 7개 이상 오버플로 탈출구로 **보존**. 문서 동기화 6곳: `deck.css` §13 주석·`an-2col` 선언 주석, `outputs/…layout-atlas.html`(프로즌 CSS), `kit/starter/deck-template.html`, `kit/guide/디자인시스템.md`, `kit/guide/토큰-치트시트.md`, `MEMORY.md`. 참고: 0.6.0·0.7.0 문서가 이 규칙을 "verify 강제"라고 적었으나 `verify_deck.py`에 해당 검사는 존재한 적이 없다 — 표현을 함께 정정했다.
- **상시 규칙 3종을 `SKILL.md` 정본으로 승격**(특정 주차·슬라이드에 종속되지 않는 서술): ① **이미지 잘림·겹침 0** — `figure`와 `img`의 렌더 높이 일치 강제(`overflow:hidden` figure에 더 큰 `img`를 넣어 하단이 잘린 채 두지 않음), 형제 텍스트 박스와 경계 상자 교차 0, **`object-fit:contain` 레터박스 함정**(가로가 넓은 그림을 세로 박스에 넣으면 높이를 키워도 빈 여백만 커진다 → **가로**를 늘려야 함). ② **페이지번호·푸터 영역 침범 0** — 콘텐츠 하단 상한 **666px**, `overflow:hidden`으로 가려 통과 금지, `getBoundingClientRect().bottom` 실측. ③ **박스 내부 세로 균형** — 캔버스의 "하단 여백 금지"를 박스 안쪽에도 적용, 항목 간격을 균등하게 유지한 채 늘려 채움(`space-between`/`space-evenly`).
- **`verify_deck.py` 1주차 계약**: 48p(S39) 뒤 완성 목표 쇼케이스 `S39A` 추가에 맞춰 슬라이드 수 어서션 71 → **72**.

## 0.7.0-wip — 대상 정의 정정·가독성 레이어 개명 (2026-07-19)
- **대상 정의 정정**: "40~50대 초보자" 표현을 커리큘럼 정본(`courses/<과목>/커리큘럼_기준안.md` §4.1)에 맞춰 "코딩 경험이 없는 입문자 중심의 혼합군(연령·경험 수준 다양)"으로 저장소 전역 정정. 수치·CSS 값은 변경 없음, 근거 문장만 교체.
