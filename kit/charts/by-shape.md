...
>
> **동점 규칙**: 레이아웃 후보가 적합도 동점일 때 element를 얹은 쪽을 먼저 고를지는 **과목 프로필 §4 「표현 취향」**이 정한다(취향이지 보편 원리가 아니다). 다만 **수치·비교 이야기를 글 카드로 풀고 차트를 하단 장식 막대로 밀어내는 것**은 취향 문제가 아니라 정보 모양 오판이다 — 그건 어느 과목에서도 잘못이다. 상세는 `../layouts/by-shape.md` 동점 규칙.
>
> ⚠️ 이미지 생성 도구가 없다는 것이 "시각자료 없음"의 이유가 되지 않는다. **화면의 시각적 풍부함은 일러스트가 아니라 차트에서 나온다** — 프로필 §4의 기준 슬라이드를 열어 확인하라. 차트는 도구 없이 만들 수 있다.
...
1. `E-code` — 소스코드 한 조각의 **특정 줄**을 라인번호 + 강조 줄 배경 + 여백 콜아웃(블루 리더선)으로 "이 줄이 뭘 하나" 풀이(초보자 대상 코드 읽기)
