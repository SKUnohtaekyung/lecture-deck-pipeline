...
- **composition_family**: `top-down`
- **composition_shape**: `band-grid`
- **variant_of**: `L-gm-quad` (구도 중복 접기 · 균형 카운트에서 `L-gm-quad`으로 집계 — 규격 §2b/§3, 본문 보존)
...
- **adaptation_note**: 2×2 사분면 골격만 차용. 원본의 컬러 스퀘어·아이콘을 복제하지 않고 `.grid-2`×2 + `.card.surface`, 칸 번호는 `.num-circle`로 재표현. 축 라벨은 `.s-eyebrow` 톤 캡션으로 칸 밖 여백에. 색은 강조 칸 1개만 블루, 나머지 뮤트 블루-패널(`.card.surface`).
...
- **composition_family**: `top-down`
- **composition_shape**: `band-grid`
- **variant_of**: `L-gm-bento` (구도 중복 접기 · 균형 카운트에서 `L-gm-bento`으로 집계 — 규격 §2b/§3, 본문 보존)
- **info_shapes**: `[classification]`
...
└───────────────┴──────────────────────────────┘
```
- **coded**: yes — `kit/layouts/catalog.html` `data-slide="L-td-glossary"`. 브라우저 검증 완료(2026-07-15): 오버플로 0(`.s-full` 548px 예산 내), 콘솔 에러 0.
...
- **capacity**: 주장 헤드라인 ≤ 16어(`.s-title` 38px 800, ≤2줄) + 근거 카드 **2~4장**(①~④, 권장 3). `.grid-3` 기준 카드폭 ≈ 360px, 카드당 소제목 ≤ 6어(**22px/800**) + 본문 ≤ 3줄(22px/1.78). 4장은 `.grid-4` 폭 ≈ 264px로 좁아져 본문 ≤ 2줄로 컷.
...
└───────────┴───────────┴───────────────────────┘
```
- **coded**: yes — `kit/layouts/catalog.html` `data-slide="L-td-claim-rationale"`. 브라우저 검증 완료(2026-07-15): 오버플로 0(`.s-full` 548px 예산 내), 콘솔 에러 0.
- **density_note**: 세로 예산 548px. **아래 수치는 브라우저 실측(2026-07-26 · `.s-full` 548.0 / top 118 확인)**이다. 밴드 = `.s-eyebrow` **32.5px** + 주장 title(`.s-title` 38px/line-height 46.36 → 1줄 **46.4px** · 2줄 **92.7px**) + accent-bar 5px + `↓ 왜냐면` 커넥터 스파인 `.cr-spine` **22.5px** + 마진 → title 1줄이면 **≈178px**, 2줄이면 **≈225px** 소비. 근거 카드 행(`.grid-3`)에 남는 값은 title 1줄 **369.6px**(실측) · 2줄 **≈323px**. 카드 `.cr-card` = 패딩 20×2 + `.timing` 마커 + 소제목 `h3`(22px/800) + 본문(22px/1.6 → 줄당 **35.2px**) → **본문 1줄 실측 156.7px**, 3줄이면 ≈227px → title 2줄·본문 3줄 최악 조합에서도 323 > 227로 **한 행에 여유(오버플로 0)**. 본문 ≥22px/keep-all, 마커·소제목·커넥터 라벨 ≥17px 하한. **swiss-columns와의 구별(중복 차단)**: swiss-columns는 대등 단(column)의 병렬 훑기(classification)라 밴드가 "한 줄 인사이트 제목"이고 커넥터·번호가 없다. 여기는 (1) 밴드가 **완결-문장 주장**으로 카드를 지배, (2) `↓ 왜냐면` **커넥터 스파인이 밴드를 카드 행에 물리적으로 잇는 종속 시각**, (3) 각 카드에 **블루 번호 마커**(=매달린 이유임을 표식)를 둔다 — 이 셋이 구조적 차별자다. **번호 마커는 구조 마커이므로 블루**(`.timing` 블루 배지; 민트 fill 배지 3종 `.num-circle`·`.work-step .n`·`.pd-dot.is-active` **아님** — 그 외 노드=블루 규칙 준수). **근거 카드는 서로 대등 → 색 차등·강조 순위 금지**: 전 카드 동일 뮤트 표면(`.card.surface`=블루-패널) + 블루 번호로 균일. 위계는 오직 밴드(주장)↓카드(근거) **배치·커넥터·번호**로 표현하고, 강조(`.hl`)는 주장 밴드 핵심어 **한 곳에만**. navy·그라데이션 없음, 민트/코랄 fill 위 흰 글자 없음, raw `#hex` 없이 토큰만.
