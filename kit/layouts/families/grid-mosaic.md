...
- **adaptation_note**: 원본 PPT 사분면은 색면 4블록. 재표현은 블루-패널 `.surface` 카드 4장을 `.grid-2`로 깔고 구조 제목·추천 기준을 `--blue`, 행동·완료 도형을 민트로 쓴다. 본문은 `--ink`, 사분면 구분은 색면 남용 대신 유색 카드 보더(`--blue-line-strong`)+간격으로 만든다.
...
- **when_to_avoid**: 위험이 1~2개뿐일 때(단일 `.callout.coral` 하나가 정직 — 밴드 격자는 과함) · 항목이 순서·절차를 가질 때(`vertical-flow`) · 대안·이유 없이 단순 경고 나열이라 세 지문 중 둘이 빌 때(`L-gm-checklist` 2열 상태 격자) · 위험이 5개↑일 때(세로 예산은 지문 1줄이면 5밴드까지 물리적으로 성립하지만 — 아래 capacity 참조 — 그 경우 위험당 이유·대안이 라벨 한 줄로 눌려 이 레이아웃의 목적인 "왜 위험한가·대신 무엇을"이 전달되지 않는다. **회피 사유는 예산이 아니라 밀도다.** 개수 축소, 또는 지문이 라벨뿐이면 `L-gm-checklist`, 정형 셀이면 `L-td-table`).
- **capacity**: 3~4 위험 밴드(세로 1열 스택). 밴드당 세 지문(위험 제목 / 이유 / 대안)이 가로 3분할, 각 지문 폭 ~365px(1152/3−gap), 본문 22px/1.78·keep-all에서 **줄당 ~16자**. **세로 검산은 타이틀 블록을 반드시 포함**한다. **아래 수치는 브라우저 실측(2026-07-26)**이다: `.s-eyebrow` 32.5 + `.s-title`(38px/line-height 46.36 → 1줄 46.4) + accent-bar 5 + 마진 = **타이틀 블록 139.9px** → 세로 예산 548px 중 밴드에 남는 실제 예산 `.pf-list` = **408.1px**(옵션 `.ca-lead`류 리드 한 줄 추가 시 −~30px). 밴드 `.pf-band`(패딩 14×2, 지문 3분할, `.pf-reason`/`.pf-alt` 19px/1.6 → 줄당 30.4) 실측 = **지문 1줄 60.4px · 2줄 90.8px**, 밴드 간 gap 10. — **3밴드(권장)**: 지문 2줄까지 여유. 3×90.8 + 2×10 = **292.4 ≤ 408.1** ✓(여유 ~116px). risk_title ≤5어 · reason/alternative 각 ≤~8어(≤2줄). — **4밴드**: 지문 2줄도 성립. 4×90.8 + 3×10 = **393.2 ≤ 408.1** ✓(여유 ~15px — 지문이 3줄로 넘어가면 즉시 초과하므로 각 ≤2줄 엄수, intro_lead 생략 권장). — **5밴드↑**: 지문 2줄이면 5×90.8 + 4×10 = **494 > 408.1** ✗. 지문 1줄로 눌러야 5×60.4 + 4×10 = 342 ≤ 408.1로 들어가지만, 그러면 이유·대안이 라벨이 되어 레이아웃의 목적이 사라진다 — **예산이 아니라 밀도를 근거로 회피**하고 4개로 축소하거나 `L-gm-checklist`/`L-td-table`로.
...
```
- **coded**: yes — `kit/layouts/catalog.html` `data-slide="L-gm-pitfalls"`. 브라우저 검증 완료(2026-07-15): 오버플로 0(`.s-full` 548px 예산 내), 콘솔 에러 0.
