...
원본 출력 보관: `tmp/hardening/baseline/`(휘발 — 수치 정본은 이 표다). 스파이크 변주 현물 58MB는 P4 판정 후 정리했고 **생성기 3개는 보존**했다(`tmp/spike/{build_variant,spike,mut_dom}.py` — 재현하려면 `python tmp/spike/spike.py`).
...
| Phase | 상태 | 커밋 |
|---|---|---|
| P0 경로 층 전제 통일 | **완료** | `8856fa5` |
| P1 0판정 가드 + 미판정 명시 | **완료 · 사용자 판정 1건 대기** | `5e014b9` · `1bd9129` |
| P2 테마 선언 축 신설 | **완료** | `c733758` |
| P3 집행부 테마 파라미터화 | **완료** | `12c16c0` |
| P4 뮤테이션 매트릭스 회귀 승격 **판정만** | **판정 완료 · 사용자 결정 대기** | (아래 기록) |
...
### P4 — 뮤테이션 매트릭스의 회귀 승격 : **판정 (실행 아님 · 사용자 결정 대기)**

**결론: 조건부 승격 권고 — 「하네스」가 아니라 「계약」을 승격한다.**

#### 실측 비용

| 대상 | 크기 | 1셀 소요 | 비고 |
|---|---:|---:|---|
| 스파이크 하네스 현물(`tmp/spike/`) | **58MB** (변주 19개 × 3.2MB) | verify_deck 395ms | 실물 3주차 덱(315KB) + kit + 자료 이미지를 변주마다 복제 |
| 기존 픽스처(`tests/fixtures/mini-week/9주차`) | **50KB** | **213ms**(조립 45 + 검증 168) | 12장 · 판정 41건(FAIL 0·WARN 3·PASS 38) |

#### 판정 근거

1. **하네스 현물은 승격 불가.** 58MB도 문제지만 결정적인 것은 **실물 3주차 덱에 결합**된다는 점이다 — 동결·수정 대상 산출물이 바뀌면 회귀가 깨진다. 테스트가 산출물을 인질로 잡는 구조는 만들지 않는다.
2. **기존 픽스처가 필요한 구조를 이미 전부 갖고 있다** — kit CSS 3종을 링크하고(상대 깊이 5), `.s-body`/`.s-lead`가 13곳 있으며, `_build_mutated()`·`_verify_rc()` 헬퍼도 `tests/test_deck_contract.py`에 이미 있다. **새로 만들 것이 거의 없다.**
3. **비용이 작다** — 핵심 4~6셀 × 213ms ≈ **1.3초**. 현재 회귀 261건 32초 대비 **약 4%**.
4. **R-PROMO-01의 취지는 충족한다.** 그 규칙은 레이아웃 구도용이지만 「1회 사용분을 예측으로 승격하지 마라」는 취지는 적용된다 — 이 하네스는 **2회 사용**됐고(스파이크 최초 측정 · P3 검증 재실행) 두 번 다 결정적 증거를 냈다.

#### 승격한다면 — 권고 범위 (전체 15셀이 아니라 핵심 4셀)

| # | 셀 | 무엇을 지키는가 |
|---|---|---|
| 1 | 팔레트 교체 + 테마 **미등재** → FAIL | F1 회귀 — 「검사가 저장소가 아니라 덱을 본다」 |
| 2 | 팔레트 교체 + 테마 **등재** → PASS | 오탐 방지 — 정당한 새 테마가 막히지 않는다 |
| 3 | 셸 어휘 교체 → FAIL | G1 어휘 동결의 집행 |
| 4 | 뮤턴트(폰트 하한 · 아이콘 누출) → 검출 | 「게이트가 아직 보고 있는가」의 최소 증명 |

15셀 전량은 같은 것을 반복 증명하고 유지비만 늘린다. **의심스러우면 한 단계 낮춘다.**

#### 승격하지 않는다면

`tmp/spike/`는 휘발이라 재현하려면 다시 만들어야 한다(생성기 `tmp/spike/{build_variant,spike,mut_dom}.py`는 보존 중, 변주 현물 58MB는 정리 대상). 그 경우 **다음에 이 질문이 나오면 처음부터 다시 측정한다** — 2026-08-23 실험덱과 같은 처분이다.

**→ 실행하지 않고 여기서 멈춘다. 승격 여부·범위는 사용자 결정.**
