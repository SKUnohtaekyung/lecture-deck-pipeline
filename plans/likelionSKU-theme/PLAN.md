...
#### ◇ 확정 기록 (2026-08-29 실행 · 실측 근거)

**◇ 4건 중 2건은 «파생»이 아니라 원본 실존값이었다** — 원본 `덱_템플릿킷/styles/deck.css`
전수 grep으로 확인했다. 즉 ◇3·◇4는 ◆로 승격된다.

| ◇ | 토큰 | 확정값 | 근거 |
|---|---|---|---|
| 1 | `--on-coral` | **#391106 (변경)** | coral이 #F97360→#F84818로 바뀌면 기존 #5C1C12의 대비가 **4.69 → 3.64**로 내려가 WCAG AA 4.5 미달이 된다. 계약 §4㉡ 「테마가 스스로 하한 미달을 생산하면 그 테마로 만든 모든 덱이 첫날부터 FAIL한다」에 정면으로 걸린다. 새 coral을 같은 색상으로 어둡게 내려(k=0.23) **4.72**로 복원 — default 테마 자신의 마진(4.69)과 같은 수준 |
| 2 | `--paper` | **#F4F8FB (유지)** | 원본에 라이트 표지 배경 **대응값이 없다**(원본 표지는 다크 #1A1A1A) → 표의 「없으면 유지」 원칙(`--mint`과 같은 처리). 잠정안 #F5F9FF는 `--blue-panel` #F5F8FF와 **1/255 차이**라 시각적 실체가 없고, 아이스 기준 재파생은 모두 기존 토큰과 충돌한다(6% 틴트=#F8FBFF=`--surface-recap` · #F5F8FF=`--blue-panel` · #F4F8FF=`--blue-panel-alt`). 대비 이득도 ≤0.2(ink 16.63→16.81)로 측정 의미가 없다 |
| 3 | `--blue-line-strong` | **#CDDBFF (유지 · ◆)** | 원본 deck.css에 **실존**(1회). 재파생이 아니었다 |
| 4 | `--blue-overlay` | **rgba(238,242,255,.92) (유지 · ◆)** | 원본 deck.css에 **실존**(1회). 게다가 이 테마에서는 성분 238,242,255 = #EEF2FF = 새 `--blue-soft`와 **정확히 일치**해 내부 정합까지 맞는다(default에서는 blue-soft가 #EEF3FE라 어긋나 있었다) |

**그림자 4종은 유지로 확정한다.** 원본은 그림자를 코발트로 틴트하지만(`rgba(48,96,195,…)` 5회 실측),
완료 기준 2가 「**색 49개만** 값 변경」을 요구하고 계약 계수상 그림자는 비색이다. 관측만 §11에 기록한다.

**실측 결과**: 토큰 99개 · 이름 집합과 **선언 순서까지** default와 동일 · 값 변경 **17개(전부 색)** ·
비색 변경 **0개**. `python -m unittest tests.test_theme_contract` → **Ran 8 tests OK · exit 0**.
...
- **(범위 밖 관측 · 고치지 않음)** 원본은 그림자를 코발트로 틴트한다(`rgba(48,96,195,…)` 5종).
  우리 default는 `rgba(29,78,216,…)`(default `--blue`) 기준이고, likelionSKU 테마는 완료 기준 2
  때문에 이를 유지했다. 그림자를 테마 축에 넣을지는 **별도 계약 갱신 안건**이다.

---

## 12. 실행 중 추기 — 계획과 현실이 어긋난 지점 (2026-08-29 실행 세션)

계획을 즉석에서 고치지 않고, 어긋남·판단·근거를 여기 남기고 진행했다.

| # | 어긋남 | 판단·근거 |
|---|---|---|
| 1 | **effort를 지정할 수단이 없다** — §6은 워커에 effort(medium/low/high)를 배정하지만, 이 하네스의 Agent 도구는 `model`만 노출하고 `effort` 파라미터가 없다 | 계획대로 `model: sonnet`만 지정하고 effort는 세션 상속값으로 실행했다. **배정 의도(판단 부하 차등)는 워커 프롬프트의 지시 밀도로 대체**했다 — B에는 격리 규칙 3종과 자체 실측 의무를, C에는 원문 대조 의무를, E①에는 어휘 계약 준수 조건을 명시 |
| 2 | **회귀 건수 정본이 두 곳에서 다르다** — `references/검증-명령-지도.md` §6은 **219건**, `MEMORY.md`는 2026-08-24 기준 **12모듈 261건** | 계획 §5 ⓒ는 219건을 인용한다. 단계 F에서 **실측 계수를 정본으로 삼고** 결과에 실제 수를 적는다. 문서 간 불일치는 범위 밖이라 고치지 않고 보고에만 남긴다 |
| 3 | **단계 0 커밋 범위** — 계획은 `PLAN.md`만 명시하지만 같은 폴더에 `PROMPT.md`(실행 프롬프트 사본)가 미추적으로 있었다 | 함께 커밋했다. 남기면 종료 시 「저장소 밖·범위 밖 미추적 파일 0건」 확인에서 상시 잡음이 된다. 같은 작업 폴더의 산출물이라 범위 밖이 아니다 |
| 6 | **§F 방침이 예상한 「호출부에 과목 명시」만으로는 부족했다** — `verify_deck_quality`·`verify_draft_quality`는 **모듈 최상위**에서 과목 종속 임계를 읽고 있었다. 최상위에는 「호출부」가 없다(import는 어느 과목인지 모르는 시점이다). 과목이 2개가 되는 순간 **import만으로** `AmbiguousCourseError`가 터져 `test_quality_gates` 53건이 통째로 무너졌다 | 적재 «시점»을 import → `run_checks()` 진입으로 옮기고 `course=` 인자를 뚫었다. 이렇게 해야 비로소 §F가 말하는 「호출부 명시」가 성립한다. 예외는 여전히 올라간다(삼키지 않음) |
| 7 | **`resolve_course`가 후보 0개인 루트에서도 던졌다** — §F 방침에 없던 정정. 구경로 합성 픽스처(`courses/` 자체가 없는 루트)를 읽는 호출이 전역 `CREATE_SLIDES_COURSE` 하나 때문에 깨졌다 | **후보 0개면 오귀속이 원리적으로 불가능**하므로(고를 남의 과목이 없다) 그 자리에서는 과목명을 무시하고 `None`(구경로 폴백)을 준다. **후보가 1개 이상인데 이름이 안 맞으면 여전히 던진다** — 오타 보호는 유지되고, 회귀 4건이 그 쌍을 고정한다. 기준을 낮춘 것이 아니라 «막을 사고가 없는 자리»를 제외한 것이다 |
| 8 | **회귀 건수 정본이 실측으로 확정됐다**(추기 2의 후속) — 지도 §6의 **219건은 낡았다** | 내 변경 전 실측 **261건**(MEMORY 기재와 일치) · 신설 5건 후 **266건**. §5 ⓒ의 「219건」은 이 수로 읽는다. 지도 §6 수정은 범위 밖이라 하지 않고 보고에만 남긴다 |
| 9 | **재검토(2026-08-29 2차)에서 그림자 결정을 뒤집었다** — §3-A는 「그림자는 계약 계수상 비색이므로 기본은 유지」였고 1차에 그대로 유지했다. 재대조에서 `rgba(29,78,216,…)`가 **default `--blue`**임이 드러났다 — 이 테마의 덱이 자기 팔레트에 없는 파랑으로 그림자를 드리우고 있었다(원본은 자기 코발트로 틴트한다) | **색상 성분만** 이 테마의 `--blue`로 옮겼다(투명도·오프셋·블러 불변). 완료 기준 2의 「색 49개만」을 문자 그대로는 벗어나지만(변경 17→20), 결정 1의 취지는 **크기·행간·간격**이었고 값 안에 브랜드 색을 품은 토큰을 «색 아님»으로 취급하면 테마가 원리적으로 완성될 수 없다. 커밋 하나로 되돌릴 수 있다 |
| 10 | **★ 등재한 테마로 정상적인 주차 덱을 만들 수 없었다** — 계약 §5 「아직 남은 것」이 예고한 「집행 이관 미완」의 실제 대가다. 정본 kit을 링크하면 `deck.css :root`가 default라 게이트가 FAIL했고, 통과시키는 유일한 길이 kit CSS 전체 사본이었다(1차 실측 때 실제로 그렇게 했다 — **드리프트 보장 장치**) | 테마 파일을 **링크 대상**으로 승격했다(`linked_theme_tokens` 신설 · 계약 §6). default 덱은 링크를 넣지 않으므로 **동결 산출물 무영향**. 시험 덱의 kit CSS 사본을 삭제하고 정본 링크로 전환해 매트릭스를 재실행했다 |
| 11 | **★ 상속되는 kit 자산을 격리 게이트가 보지 않았다** — 스캔 범위가 문서(.md)와 테마 선언뿐이라, 복사되는 스타터·링크되는 공용 CSS는 사각지대였다. 공용 kit에 한 과목 브랜드가 **31곳** 있는 동안 게이트는 PASS였다(미탐) | `SCAN_WARN`에 `kit/starter/*`·`kit/styles/*.css` 편입(기준: 「내용이 새 과목 덱으로 상속되는가」). 리터럴을 전량 제거한 뒤라 신규 WARN 0. kit이 깨끗해져 `SKU LIKELION`을 프로필 §8에 **등재**할 수 있게 됐다 — 「등재하면 FAIL이니 제외」가 아니라 **「FAIL 원인을 없애고 등재」**가 옳은 방향이었다 |
| 5 | **단계 0의 `.gitignore` 패턴이 우리 산출물까지 삼켰다** — 계획 §3-0은 「`.gitignore`에 `likelionSKU/` 추가」라고만 적었는데, 앵커 없는 패턴은 **어느 깊이의 동명 폴더든** 잡는다. 단계 A 커밋 시도에서 `kit/themes/likelionSKU`가 `paths are ignored`로 거부되며 실측 발견 | `/likelionSKU/`로 **앵커해 고쳤다**(별도 fix 커밋 · §9 커밋 계획에 없던 1건 추가). 원본 폴더는 여전히 차단되고(`git check-ignore` 매치) 우리 산출물 2곳은 추적 가능(exit 1). **`courses/likelionSKU/`도 같은 함정에 걸려 있었으므로 워커 B·C 산출물이 조용히 사라질 뻔했다** — 커밋 3 전에 잡아서 유실은 없다 |
| 4 | **B·C 워커 착수 시점** — §3 순서는 A → B∥C지만, B·C는 A의 산출물에 의존하지 않고 파일도 겹치지 않는다(단일 라이터 충족) | A와 동시에 착수했다. **게이트 순서는 계획대로 유지**한다 — A 검증 → B 검증 → 커밋 2 → 커밋 3 |
