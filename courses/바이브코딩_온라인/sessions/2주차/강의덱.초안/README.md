# 강의덱.초안 — 바이브코딩 온라인 원데이 · 2차시 편집본 조각(fragments)
...
- ⚠️ **로드 순서가 1차시 덱과 반대다.** 이 셸은 `<style id="kit-additions">` 다음에 `revision.css`를 로드한다(1차시 덱은 `revision.css` 다음 kit-additions). 1차시 조각의 CSS를 옮길 때 같은 명시도 규칙의 승패가 뒤집힐 수 있다.
- 조각 담당: `part-01`·`part-02` W1 · `part-03`·`part-05` W2 · `part-04` W3 · `part-06`·`part-07` W4. `shell.html` 고정 장은 W1.
- **`revision.css`는 메인 단일 라이터다.** 웨이브는 전용 CSS를 저장소 `tmp/vco-2주차/wave-N.css`로 내고 메인이 «2차시 이식» 구획으로 병합한다. part 파일 안에 `<style>`을 두지 않는다.
...
- CSS 경로 `../../../../kit/styles/…`는 **출력 위치 `courses/바이브코딩_온라인/sessions/2주차/강의덱.html` 기준 4단계**이고, `강의덱.초안/revision.css`도 같은 출력 위치 기준이다.
  ...
  - ⚠️ CSS를 기계 편집하면 브라우저 `document.styleSheets[i].cssRules.length`를 전후 대조한다(1차시 덱 `:is(…)` 절단 사고 — verify_deck은 FAIL 0으로 통과했다).
