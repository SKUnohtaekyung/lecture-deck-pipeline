...
| `part-04.html` | PART 4 · 4교시 · **화면과 핵심 기능** (`P4` + `M5` + 본편) | 24 |
| `part-05.html` | PART 5 · 5교시 · **AI 작업 시스템** (`P5` + `M4` + 본편) | 21 |
...
> 위 장수는 2026-08-03 머지 시점에 조각에서 **실측**한 값이다. 머지 전 두 갈래가 각각 part-03 15 / part-04 23 / part-05 22·23으로 적고 있었는데 셋 다 실제와 달랐다 — **표를 손으로 갱신하지 말고 조각에서 세라.**

**2026-08-02 슬라이드수정요청 3차**(당시 111장 시점 기록): 옛 108장(PDF) 기준 지적 반영 — 삭제 2(`C1-7`·`C1-11P`) · 순서 변경 4(`C1-N1`↔`C1-1` · `C1-6`→실습①앞 · `C2-N3`→`C2-8`뒤 · `C3-N4`→3교시→5교시 `C4-15`뒤) · 문구 정정 다수. 113 → 111장. 신규 family `w2-flow3`(`C2-N3`) 1종 추가 등재. 상세는 `deck.contract.json`의 `decks.강의덱.note`. ※ 그 뒤 수정요청 v2·08-03 작업으로 현행은 **110장**이므로, 이 문단의 파트별 장수는 당시 값으로만 읽는다.
...
- **2026-07-27 샘플 재설계 신규 구도 6종**(`w2-target`·`w2-typecard`·`w2-analogy`·`w2-itr`·`w2-handoff`·`w2-stepwarn` — p7/p13/p14/p19/p35/p58)도 같은 방식으로 `shell.html` kit-additions + `verify_deck.py` family_signature()에 등재했다. 배경: `_dev/설계기록/품질감사-2026-07/week1_visual_benchmark.md`의 S3CE/S3PRV/29/S3ITR/S3HUM/S3MAP 원리를 이식.
