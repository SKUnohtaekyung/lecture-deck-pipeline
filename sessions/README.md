...
**현행 1주차 산출물은 커밋 `95438ae`부터의 신규 계보이고 정본이다** — `초안.md`(무접두어 레거시)·`강의덱.초안/`·`강의덱.html`(75장·divider 6)·`강의덱_배포.html`·`강의덱_발표자노트.html`이 모두 실재한다.

> ⚠️ **2026-07-26 사용자 결정으로 1주차는 동결됐다. 수정·추가·삭제·재조립하지 않는다.** 읽기와 저장소 밖 복사만 허용한다.
> 구조 계약은 `sessions/_contracts/1주차.deck.contract.json`에 있다.
