...
- **② 개발자료 `_dev/`(배포 제외)**: `설계기록/`(빌드·결정 기록 · `탐색-아카이브/` 미채택본 · `품질감사-2026-07/` 2주차 저밀도 사고 감사와 재설계 기록 76파일 — `verify_deck_quality.py`·`verify_draft_quality.py`의 임계값 도출 근거가 여기 있다. 2026-08-04에 루트 `reports/`에서 이관, 구경로 대응표는 그 폴더 README). **커리큘럼·콘텐츠 원천은 `courses/<과목>/`로 이관**됨 — 상위 기준 `courses/바이브코딩/커리큘럼_기준안.md` + 주차 상세 `courses/<과목>/sessions/N주차/N주차_강의안설계.md`(옛 인수인계서 역할). `_dev/강의자료/`는 더 이상 쓰지 않는다.
...
- **장을 지우거나 옮겼으면 딸려 오는 6종을 확인한다**(노트 밀림·divider 번호 단조성·
...
- **수정 라운드를 마칠 때 「이번 지적 중 규칙층에 올릴 것」을 판정한다**(R-FEEDBACK-01 ·
  ...
  다음 주차가 상속하지 못한다. 승격 전에 **기존 지표와 충돌하는지 함께 본다.**
...
- **한 장 참조표(크기·간격·자리): `kit/guide/한장-참조표.md`** — 매번 눈대중으로 다시 정하지 않기 위한 색인
