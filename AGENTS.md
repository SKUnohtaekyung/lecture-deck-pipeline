...
- **② 개발자료 `_dev/`(배포 제외)**: `강의자료/`(콘텐츠 원천) · `설계기록/`(빌드·결정 기록 · `탐색-아카이브/` 미채택본).
...
- 40장 이상의 덱처럼 독립 구간이 큰 작업은 협업 도구가 제공될 때 병렬 서브에이전트를 사용한다. 메인 에이전트가 먼저 전역 결정표(정보 모양·시그니처·element·힌트)를 확정하고, 빌더에는 해당 결정행과 원문만 전달한다. 검증 담당은 산출물을 수정하지 않고 읽기 전용으로 점검한다. 마지막 통합과 회귀 검증은 메인 에이전트가 책임진다.
