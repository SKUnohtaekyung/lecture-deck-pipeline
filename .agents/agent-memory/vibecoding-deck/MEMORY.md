...
- **정본 2층**: 기본 발동 규칙 = `AGENTS.md` 「하네스」 절(매 세션 로드되는 유일한 정책층) / 상세 프로토콜 = **`skills/하네스/SKILL.md`**(+ `.claude` 어댑터, `/하네스` 명시 호출). Codex 어댑터·`verify_skill_setup` 등록은 **의도적으로 미도입**(그 루프는 description에 "명시 호출" 마커를 요구해 기본 발동과 충돌).
...
- ✅ **파이프라인 정보량 10x 재설계 완료**(2026-07-18, 전용 섹션 참조) — 전 회귀 PASS(`verify_skill_setup`·`verify_session_docs`·`verify_deck` FAIL 0·eval JSON·죽은 참조 0). 후속: 실전 주차(2주차~)에서 **개념KB·리뷰 HTML 첫 산출**로 실검증.
- ✅ **하네스 제도화 완료**(2026-07-18, 전용 섹션 참조) — `AGENTS.md` SOP + `skills/하네스/SKILL.md` + `.claude` 어댑터. **미도입(의도적)**: Codex 어댑터·`skills/README.md` 등재·`verify_skill_setup` 등록(마커 충돌). 필요해지면 추가.
...
- **미커밋 변경 다수** — 재설계+하네스 전체가 작업 트리에만 있다. verify 통과 상태이므로 **복구 지점 커밋 필요**.
