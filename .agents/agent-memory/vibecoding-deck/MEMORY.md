...
## 이 스킬이 하는 일
채워진 슬라이드 계획서(MD) → 1280×720 HTML 웹덱 조립(입문자 대상). 정보 모양을 먼저 판단해 레이아웃/차트를 고른다. 코드 시각화 우선, "좌 글/우 시각" 쏠림 금지(공급+측정으로).
...
## 미해결 (상태·TODO 정본)
- 📌 **작업 교훈 3건(2026-07-19 적대적 검토에서 도출)**
  1. **다축 정의를 한 축으로 접지 마라.** 기준안 §4.1은 "혼합군 · 연령과 경험 수준이 **다양**"인데, 타겟 정합에서 "40~50대"를 "코딩 경험이 없는 입문자"로 바꾸면서 **연령 하드코딩을 경험 하드코딩으로 바꿔놓기만** 했다. 축을 하나 고르려는 충동이 오류였다. 대상 표현은 세 형태 모두 **`혼합군`을 포함**해야 한다(`kit/guide/교육원칙-요약.md` §1이 정본).
  2. **문자열 스윕은 표기 변형을 전부 훑어라.** `40~50|40-50|40대|중장년`으로 42파일을 고쳤는데 `adults aged 40–50`(영문·엔대시)과 `40s 하한`(축약)이 남았다. 통합 패턴 `40~50|40-50|40–50|40s|중장년|aged 40`을 쓴다.
  3. **규칙은 "결정 지점"에 도달해야 한다.** 원칙 9를 `주석스크린샷.md`(element 문서)에만 넣었는데, 조립자는 `by-shape.md`(역인덱스)를 **먼저** 읽고 host를 고른다 — 금지 규칙이 도착하기 전에 금지된 선택이 끝났다. 규칙을 바꾸면 **역인덱스·복사 원본(`데모_제작규칙.html`)·스타터**까지 같이 훑는다.
  - ⚠️ **정본이 바뀌면 대조할 하위 문서 목록이 없다.** 기준안 §6 원칙 9에서 GIF를 지웠을 때 `1주차_강의안설계.md` 3곳이 그대로 남았다. 정본 수정 시 대조 대상(주차 강의안설계 §6·§14 · `kit/screenshots/` · `kit/layouts/by-shape.md` · `references/이미지-스크린샷-배포.md` · `kit/guide/교육원칙-요약.md` · 콘텐츠·검토 SKILL)을 훑는다.
- 🔴 **기준안 정합 작업 진행 중(2026-07-19)** — 커리큘럼 정본 `sessions/바이브코딩_커리큘럼_기준안.md`와 스킬 전반을 대조해 정정 중이다. **뿌리 원인: 덱 스킬이 기준안을 read-path에 갖고 있지 않았다**(리서치 스킬만 읽었음) → 2단계에서 추가. **0~3단계 완료(커밋 2b6371f·3687b71·e14e594·989b912·4b61626·6d5894a·bfdc6ea·6aa6fdc, 2026-07-19).**
  - ①타겟을 §4.1 **혼합군**으로 정합(42파일 · `legibility-40s.css`→`legibility.css` 개명). 표현 정본은 `kit/guide/교육원칙-요약.md` §1 — 세 형태 모두 `혼합군`을 포함한다.
  - ②`screen-operation` 규칙(§6 원칙 9): 한 슬라이드=조작 단계 1개 · 밀도 하한 예외 · **단계≠지점**(한 단계 안 지점 1~3개는 정상) · 조작 host는 `L-dc-annotated`/`L-dc-hero` 둘뿐. 단 **단계 목록 개요 슬라이드에는 `L-vf-numbered-stack`이 정답**. 역인덱스·배포참조·taxonomy·데모·1주차 강의안설계까지 전파 완료(GIF·녹화 저장소 0건).
  - ③기준안 §6 원칙 9에서 GIF·녹화 삭제(정적 캡처 통일 — 사용자 결정). ④E-code 견본 Python/Streamlit→HTML/CSS/JS(§5.1 위반이었음, charset 포함).
  - ⑤**킷 자립성 복구**: read-path 1순위를 배포 제외인 `sessions/`가 아니라 `kit/guide/교육원칙-요약.md`(배포 대상)로. 정본은 "이 저장소에서는 그쪽이 이긴다"로 조건부.
  - ⑥고아 CSS 31종 `patterns.css` 승격 + `scripts/verify_kit.py` 게이트 신설(카탈로그 클래스는 kit CSS에 정의돼야 함 · 인라인 `<style>` 금지 · 우회 3종 차단 · `EXEMPT` 빈 집합 테스트).
  - **다음은 4단계부터**: 4단계 `verify_deck.py` 재설계(결함 5종 상세는 이 절의 🔴 **"`verify_deck.py` 신뢰도 결함"** 항목) → 5단계 차트 코어 8 선정·구현 → 6단계 파이프라인 계약(🔴 **"2주차 착수 전 반드시 막을 것"** 항목) → 7단계 개념 깊이 3단계 필드(§6 원칙 4)+1주차 35청크 소급.
...
- 🔴 **2주차 착수 전 반드시 막을 것(2026-07-19 적대적 검토에서 확인)** — 아래는 "조용히 통과해서" 위험한 것들이다.
  - ✅ **G7 재대조 원문에 기준안 §11·큰 설계 절 추가 완료** — 종전 목록이 주차 상세 `§14·§15`뿐이라, 주차 상세가 없는 **2·3주차는 재대조 대상이 0개**가 돼 G7이 통째로 무력화됐다. G7은 1주차 9개념 누락 사고를 막으려 만든 게이트인데 정작 다음 주차에서 안 돌아가는 상태였다.
  - ✅ **`/검토` 대조 앵커 5개 전부 죽어 있던 것 정정 완료** — `"# 4. 교시별 콘텐츠 리서치 요청"`·`"# 2. 이미 확정된 사항"`·`"리서치 완료 기준"`·`"주요 수강생 특성"`·`"화면 자료 원칙"`이 현행 강의안설계에 **존재하지 않는 표제**였다. 표제 이름 대신 `[총괄 확정]` 태그·`#### C. 리서치·검증 요청`처럼 잘 안 변하는 앵커로 교체했다.
  - ⬜ **출처레지스트리 — 방향은 "계약 외"로 확정, 검증 스크립트만 미정합**(2026-07-19). `skills/검토/SKILL.md` 1단계가 이를 **계약 외 산출물로 명시하고 검토 대상에서 제외**했고, `skills/리서치/SKILL.md`·`skills/README.md`는 애초에 언급이 0건이다. 그런데 **`scripts/verify_session_docs.py`만 여전히 필수로 요구**한다(`:322` `files["registry"]`, `:337` 없으면 FAIL). ⇒ **남은 일은 등재가 아니라 그 반대다** — `verify_session_docs.py`에서 registry를 필수에서 내려 선택(SKIP)으로 바꾼다. 안 그러면 2주차는 계약을 100% 이행해도 verify가 FAIL한다(스크래치패드 실증됨). ⚠️ 과거 메모에 "파일 6으로 등재"라 적혀 있었으나 **폐기된 방향**이다.
  - ⬜ **5~7분 복구 기준이 필드로 없다** — 기준안이 못박은 유일한 정량 기준인데 리서치 13항목·자기검증·검토 어디에도 없고 `5~7분` 문자열이 저장소에 딱 1곳(산문 괄호)뿐이다. 2주차 실습 검증에서 판정 기준 없이 추측으로 채우게 된다.
  - ⬜ **`_template`에 `N주차_강의안설계.md` 슬롯 없음** · **`콘텐츠_리뷰.html`이 계약표·ALLOWLIST 미등재**(계약 준수가 게이트 위반을 유발) · **`sessions/README.md` 필수/선택 뒤집힘**.
...
- **카탈로그 방침 확정(2026-07-19)** — `catalog.html`을 54/23 전부로 채우지 않는다. **`by-shape.md`의 54개는 추천 어휘집이고, 실제 코드는 코어 12개(레이아웃)+코어 8개(차트)만 두고 거기서 변형**한다. 따라서 "미완"이 아니라 의도된 설계다. 단 **전제가 있다: 코어는 완전해야 한다** — ✅ **고아 CSS 31종 승격 완료(2026-07-19, 3단계)** — 카탈로그 인라인 `<style id="kit-additions">`에만 있던 정의를 `patterns.css`로 옮기고 인라인 블록을 제거했다. 이제 카탈로그를 `<section>`째 복사하면 그대로 렌더된다. 차트는 코어 8개 선정·구현이 남았다(5단계).
- 🔒 **`scripts/verify_kit.py` 게이트 신설** — ①카탈로그 마크업 클래스는 전부 kit CSS에 정의돼야 한다 ②카탈로그에 인라인 `<style>` 금지. 카탈로그·`patterns.css`를 건드렸으면 실행한다. **새 구도를 짰으면 CSS를 덱 로컬 `<style>`이 아니라 `patterns.css`로 환류**한다(루트 `SKILL.md` §5 환류 규칙) — 안 하면 주차마다 같은 CSS를 다시 짜게 되고 노드 스타일이 갈린다.
