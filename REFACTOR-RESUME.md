...
| 8 확대 검증·판정 | **조건부 적용** | (이 커밋) |

## 리팩터링 완료 — 최종 판정 **조건부 적용**

Phase 0~8 종료. **main 병합·push는 승인되지 않았고 수행하지 않았다.**
판정 근거 전문: `_dev/설계기록/Phase8-확대검증-판정.md`

**남은 사용자 결정 (전부 미도입 유지 중)**

1. **Hook(PreToolUse) 도입 여부** — 근거가 강해졌다. 2주차 재생에서 워커 19/20이 `Bash`를 썼고
   신규 구조도 이를 구조적으로 막지 못한다. 반대 근거(Codex 폴백 불가)는 그대로.
2. **전역 URL 레지스트리** — per-worker 검사가 워커 **간** 중복을 원리적으로 못 잡음이 실증됐다.
   단 중복률 9.0%로 2차 요인.
3. **§0.5 중단선 재산정** — 표본을 늘리려면 3청크당 약 4M 처리 토큰.
4. **D2 워커 턴 상한** — `maxTurns`는 `.claude/agents/` frontmatter 전용이라 이 경로에 없다. 미해소.
